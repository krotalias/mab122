/** @file Arcball/Arcball.cpp
 *
 *  Arcball algorithm works by defining a hemisphere inscribed in screen area, 
 *  where x axis is along the screen width and y axis is along the screen height. 
 *  Also remember that in OpenGL the origin of pixel coordinates (0,0) is in the upper left corner of the window.
 *
 *  - To Compile:
 *    - g++ -I/usr/include/eigen3 -c Arcball.cpp -o Arcball.o
 *  - or, for MacOS:   
 *    - g++ -I/opt/local/include/eigen3 -c Arcball.cpp -o Arcball.o
 *
 * \defgroup GLVisualization Arcball Implementation Without Quartenions
 *
 *  @author Carlo Nicolini (linello)
 *  @date 07/12/2017
 ...
 * Here you must give directly the coordinates of the mouse as the glut functions extract. 
...
/**
 * \ingroup GLVisualization
 * Maps the mouse coordinates to points on a sphere, if the points lie outside the sphere, 
 * the z is 0, otherwise is \f$ \sqrt{r^2 - (x^2+y^2)} \f$ where \f$ x,y \f$
...
/**
 * \ingroup GLVisualization
 * Set the current transformation to the given rotation matrix.
 *
 * This is an extension added to the original code, so
 * one can initialize arcball to a given orientation (position).
 *
 * @param matrix a modelView matrix.
 */
void Arcball::setViewMatrix(GLMatrix matrix)
{  
   // set the rotation part of the current matrix to the given matrix.
   startMatrix[0] = matrix[0];
   startMatrix[1] = matrix[1];
   startMatrix[2] = matrix[2];
   
   startMatrix[4] = matrix[4];
   startMatrix[5] = matrix[5];
   startMatrix[6] = matrix[6];
   
   startMatrix[8] = matrix[8];
   startMatrix[9] = matrix[9];
   startMatrix[10] = matrix[10];
}
