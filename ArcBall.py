...
class ArcBallT(object):
    ...
    def __str__(self):
        ...
        str_rep += "\n scale coords %f %f" % (
            self.m_AdjustWidth, self.m_AdjustHeight)
        return str_rep

    def setBounds(self, NewWidth, NewHeight):
        # Set new bounds
        assert (NewWidth > 1.0 and NewHeight >
                1.0), "Invalid width or height for bounds."
...
def Matrix4fSetRotationFromMatrix3f(NewObj, three_by_three_matrix):
    ...
    NewObj = Matrix4fSetRotationScaleFromMatrix3f(
...
def Matrix3fSetRotationFromQuat4f(q1, dim=3):
    ...
    NewObj[0][0] = 1.0 - (yy + zz)
    NewObj[1][0] = xy - wz
    NewObj[2][0] = xz + wy
    NewObj[0][1] = xy + wz
    NewObj[1][1] = 1.0 - (xx + zz)
    NewObj[2][1] = yz - wx
    NewObj[0][2] = xz - wy
    NewObj[1][2] = yz + wx
    NewObj[2][2] = 1.0 - (xx + yy)
